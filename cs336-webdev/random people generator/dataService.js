    ...
    async fetchData() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                jsdata.results.forEach(results => {
                    const { title, first, last } = results.name
                    const gender = results.gender
                    const address = results.location.street.number + ' ' + results.location.street.name + results.location.city + results.location.state;
